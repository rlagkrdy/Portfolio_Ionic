...
describe('YoProfileComponent', () => {
    ...
    const file = dataURLtoFile(base64, 'hello.png');
    ...
    it('이미지 클릭시 input file클릭 이벤트가 발생해야 한다.', () => {
        ...
        const imgEl: DebugElement = fixture.debugElement.query(
    ...
    it('파일 선택시 imgageChange이벤트 발생해야 한다', () => {
        const imageChange: jasmine.Spy = spyOn(component, 'imageChange');
        const fileEl: DebugElement = fixture.debugElement.query(
            By.css('#profile_file')
        );

        fileEl.triggerEventHandler('change', null);
        fixture.detectChanges();
        expect(imageChange).toHaveBeenCalled();
    });

    it('파일 선택시 file이 있을 때에는 setImgPreview이벤트를 실행한다.', () => {
        const previewEvent: jasmine.Spy = spyOn(component, 'setImgPreview');
        component.imageChange(mock);
        expect(previewEvent).toHaveBeenCalled();
    });

    it('setImgpreview이벤트가 발생하면 FileReader가 발생해야 한다.', () => {
        spyOn(<any>window, 'FileReader').and.returnValue({
            onload: (e: any) => {},
            readAsDataURL: function() {}
        });
        component.setImgPreview(fileArr);
        expect(window['FileReader']).toHaveBeenCalled();
    });

    it('파일 선택시 file이 없을 때에는 changeToOriImg이벤트를 실행한다.', () => {
        const oriEvent: jasmine.Spy = spyOn(component, 'changeToOriImg');
        component.imageChange({ target: { files: new Array<File>() } });
        expect(oriEvent).toHaveBeenCalled();
    });

    it('changeToOriImg이벤트가 발생하면 imgUrl의 값은 oriUrl의 값과 같아야 한다.', () => {
