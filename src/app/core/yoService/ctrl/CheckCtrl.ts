...
export class CheckCtrl {
    ...
    init(
        ...
        _searchArr
            ...
            .forEach(checkArr_item => {
                _func(
                    _form,
                    checkArr_item,
                    _matchk.filter(
                        matchk_item => matchk_item.name === checkArr_item.name
                    )
                );
            });
    }
    ...
    set(
        _form: NgForm,
        checkArr_item: SearchObj,
        targetArr: Array<MatCheckbox>
    ): void {
    ...
    get(
        ...
        targetArr.forEach(target_item => {
            chkValue += target_item.checked
                ? parseInt(target_item.value, 10)
                : 0;
        });
