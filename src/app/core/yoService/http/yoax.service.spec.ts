...
describe('YoaxService', () => {
    ...
    it('typeIsCorrect() :: type이면 true 아니면 false', inject(
    ...
    it('setReqContentType() :: Content-Type을 가지고있는 Headers객체를 return 받고 put일때와 아닐 때로 나뉜다.', inject(
        [YoaxService, ParamUtils, RegexUtils],
        (yoax: YoaxService, param: ParamUtils, regex: RegexUtils) => {
    ...
    it('setReqParam() :: RequestOptionsArgs객체를 return 받고 params, body에 파마미터를 담는다.', inject(
        [YoaxService, ParamUtils, RegexUtils],
        (yoax: YoaxService, param: ParamUtils, regex: RegexUtils) => {
    ...
    it('fileYoax() :: Observable객체를 return 받는다', async(
        inject(
            [YoaxService, ParamUtils, RegexUtils],
            (yoax: YoaxService, param: ParamUtils, regex: RegexUtils) => {
                yoax.fileYoax('/usr/imageInsert', fileArr).subscribe(event => {
